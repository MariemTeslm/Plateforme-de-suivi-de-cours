...
async function loadDropdowns() {
    try {
        const [groups, profs, matieres, salles, periodes, activeWeekRes] = await Promise.all([
            API.get("/groupes/all"),
            API.get("/profs/all"),
            API.get("/matieres/list"),
            API.get("/salles/all"),
            API.get("/periodes/all"),
            API.get("/semaines/active")
        ]);

        const fillSelect = (id, data, textKey, valueKey = textKey) => {
            const select = document.getElementById(id);
            if (!select) return;
            select.innerHTML = '<option value="">-- Sélectionner --</option>'; // Clear first
            data.forEach(item => {
        ...
        const activeWeek = activeWeekRes.data;
        if (activeWeek) {
            const weekInput = document.getElementById('id_semaine');
            if (weekInput) {
                weekInput.value = activeWeek.id;
                const existingHint = document.getElementById('active-week-hint');
                if (existingHint) existingHint.remove();

                const hint = document.createElement('span');
                hint.id = 'active-week-hint';
                hint.style.cssText = 'color:#2563eb;font-size:0.85rem;font-weight:500;margin-top:4px;display:block;';
                const start = new Date(activeWeek.date_debut).toLocaleDateString('fr-FR');
                const end = new Date(activeWeek.date_fin).toLocaleDateString('fr-FR');
                hint.textContent = `Semaine ${activeWeek.numSemaine} – du ${start} au ${end}`;
                weekInput.after(hint);
                window.activeWeekId = activeWeek.id;
            }
        }
        await loadSeances();
